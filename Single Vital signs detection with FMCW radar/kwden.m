function [xd,cxd,lxd,thrs] = kwden(in1,in2,in3,in4,in5,in6,in7,in8)
% 经 Mr.看海 修改后的小波阈值滤波函数（原函数为wden）
% 加入了 visushrink 方法，即统一阈值去噪方法
% 加入了改进阈值函数，具体见kwthresh.m文件
% 加入了为满足改进阈值函数额外的调节因子的入口参数in8
...
if nargin > 2 && isStringScalar(in2)
...
if nargin > 4 && isStringScalar(in4)
    in4 = convertStringsToChars(in4);
end

if nargin > 5 && isStringScalar(in5)
    in5 = convertStringsToChars(in5);
end

if nargin > 6 && isStringScalar(in6)
    in6 = convertStringsToChars(in6);
end

if nargin > 7 && isStringScalar(in7)
...
switch nbIn
    ...
    case 7
        ...
        scal = in4; n = in5; w = in6; options = in7;
    case 8
...
for k = 1:n
    ...
    if strcmp(tptr,'sqtwolog') || strcmp(tptr,'minimaxi')
        thr = kthselect(c,tptr);
    elseif strcmp(tptr,'visushrink')
        thrtemp = kthselect(x,tptr,n,w);
        thr = thrtemp(k);
    else
        if s(k) < sqrt(eps) * max(c(flk))
            thr = 0;
        else
            thr = kthselect(c(flk)/s(k),tptr);
        end
    end                                     % threshold.
    thrs(k)      = thr * s(k);                  % rescaled threshold.
    cxd(flk) = kwthresh(c(flk),sorh,thrs(k),options);    % thresholding or shrinking.
end
